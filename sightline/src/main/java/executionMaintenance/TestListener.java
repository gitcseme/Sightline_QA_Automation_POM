...
public class TestListener implements ITestListener {
	...
	public void onStart(ITestContext context) {
		...
		if (!context.getName().equalsIgnoreCase("Input")) {
			...
			try {
				...
				JSONArray issueIds = new JSONArray(issues); // Issue Id's to be added to Test Cycle, add more issueKeys
															// separated by comma
			...
			try {
				...
				for (; offset <= tcCount;) {
					executionIds.putAll(
							ZapiScripts.getExecutionsByCycleId(getExecutionsUri, client, accessKey, tcCount, offset));
					offset = offset + 50;
				}
	...
	public void onTestSuccess(ITestResult Result) {
		...
		for (String issue : issues) {
			...
			String getStatusURI = API_UPDATE_EXECUTION.replace("{SERVER}", zephyrBaseUrl) + execId + "?issueId="
					+ JIRAissueID + "&projectId=" + projectId;
			String status = "";
			try {
