...
//		//login as sa
	...
	////
	/**
	 * @author N/A Testcase No:RPMXCON-48649
	...
	public void verifyDoNtProdPDFForNativePH() throws Exception {
		base.stepInfo("RPMXCON-48649");
		base.stepInfo("To verify that if \"Do Not Produce PDFs for Natively Produced Docs\" is disabled "
				+ "then PDFs is generated for all docs being produced.");
		productionname = "p" + Utility.dynamicNameAppender();
		int doc = 1;
		String prefixID = "A_" + Utility.dynamicNameAppender();
		String suffixID = "_P" + Utility.dynamicNameAppender();
		...
		String beginningBates = page.getRandomNumber(2);
		int FirstFile = Integer.valueOf(beginningBates);
		int secondFile = FirstFile + doc;
		int lastFile = FirstFile + doc + doc;
		...
		sessionSearch.addNewSearch();
		sessionSearch.removeAllAddedTiles();
		sessionSearch.newMetaDataSearchInBasicSearch("DocFileType", "Pdf");
		sessionSearch.ViewInDocList();
		DocListPage doclist2 = new DocListPage(driver);
		doclist2.documentSelection(doc);
		doclist2.bulkTagExistingFromDoclist(tagName);
		...
		base.waitForElement(page.getTIFFChkBox());
		page.getTIFFChkBox().waitAndClick(2);
		base.waitForElement(page.getTIFFTab());
		...
		File pdfFile1 = new File(home + "/Downloads/" + "VOL0001/PDF/0001/" + prefixID + FirstFile + suffixID + ".pdf");
		File pdfFile2 = new File(
				home + "/Downloads/" + "VOL0001/PDF/0001/" + prefixID + secondFile + suffixID + ".pdf");
		File pdfFile3 = new File(home + "/Downloads/" + "VOL0001/PDF/0001/" + prefixID + lastFile + suffixID + ".pdf");
		File DatFile = new File(home + "/Downloads/VOL0001/" + name + "/" + name + "_DAT.dat");

		if (pdfFile1.exists() && pdfFile2.exists() && pdfFile3.exists() && DatFile.exists()) {
			base.passedStep("PDF Generated For All Doc along with Dat File");
		} else {
			base.failedStep("PDF For All Doc along with Dat File Not Generated");
		}
		base.passedStep("verified - that if \"Do Not Produce PDFs for Natively Produced Docs\" is disabled "
				+ "then PDFs is generated for all docs being produced.");
		loginPage.logout();
	}
