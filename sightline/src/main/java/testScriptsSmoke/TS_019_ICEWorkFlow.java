...
public class TS_019_ICEWorkFlow {
	...
	public void PMUploadAndInitiate() throws InterruptedException
	{
	 //The below line of codes will make sure the RMU user has access to dataset option.
		...
		Assert.assertEquals(mup.getInitatePopupMessage().getText(), "Are you sure you want to initiate processing for this dataset?");
		...
		driver.WaitUntil((new Callable<Boolean>() {public Boolean call(){return 
				dpdp.getProcessingStatus().equalsIgnoreCase("Processing"); }}),10000);
	...
	public void RMUploadAndInitiate() throws InterruptedException
	{
		...
		driver.waitForPageToBeReady();
    	 ...
    	 //The below line of codes will make sure the RMU user has access to dataset option.
    	 //For the current project, also verifies that by default RMU user has no access.
