...
public class TagsAndFolders_23 {
	 ...
	 * @Description:Verify that if RMU delete the Tag which is assigned to multiple SG,it should deleted only from the current selected SG only
	 **/
	 ...
	 * @Description:Verify that if RMU delete the Folder which is assigned to multiple SG,it should deleted only from the current selected SG only
	 **/
...
//	    asserts.assertAll();
	...
	public void verifyMD5HashNoTagProp() throws Exception {
		...
		base.stepInfo("Verified - that if the doc being applied to has no MD5Hash, then no Tag propagation should happen");
		loginPage.logout();
	 ...
	 * @Description:Verify that error message does not display and application accepts - when new Tag name entered with * ; ‘ / ( ) # & ” from Doc Explorer > bulk Tag
	 **/
	...
	public void verifyErrorMsgWithSpecialChInTagCreation() throws Exception {
		...
		base.stepInfo("Verify that error message does not display and application accepts - when new Tag name entered with * ; ‘ / ( ) # & ” from Doc Explorer > bulk Tag");
		...
		if (tagsAndFolderPage.getErrorMsgPopup().isElementAvailable(5)) {
			base.passedStep("Verify that error message does not display and application accepts - when new Tag name entered with * ; ‘ / ( ) # & ” from Doc Explorer > bulk Tag");
		}
		else {
